 ...
 *
 * The Castmate G is a broadcast tag, so its effective rate is its advertising
 * interval (motion-triggered), not a sampling rate we set. ~10 Hz assumes the
 * fastest interval (~100 ms) the tag is configured for. Detection windows are
 * expressed in seconds, so they scale correctly if this changes; but note a
 * coin-cell beacon cannot match a wired 50 Hz IMU, so bite waveforms are coarser
 * than a dedicated sensor would provide.
 */
